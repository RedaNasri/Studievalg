      ...
      <main className="min-h-screen" style={{background: '#F6F9FC'}}>
        <div className="max-w-4xl mx-auto px-6 py-12">
          ...
          <div className="bg-white rounded-2xl p-6 mb-6" style={{border: '1px solid #E4E9F2', boxShadow: '0 1px 2px rgba(13,27,42,0.04), 0 4px 12px rgba(13,27,42,0.04)'}}>
            ...
            <button onClick={() => { if (bachelor && karakter) { setSokt(true); setTimeout(() => { document.getElementById('resultater-master')?.scrollIntoView({ behavior: 'smooth' }) }, 300) } }} className="w-full text-white py-4 rounded-xl font-semibold text-base transition sticky bottom-4" style={{background: '#0D1B2A'}}>
...
const gradeOrder: any = { 'A': 5, 'B': 4, 'C': 3, 'D': 2, 'E': 1, 'F': 0 }
    ...
    function getStatus(m: any) {
      const kategori = bachelorTilKategori[bachelor]
      const bachelorMatch = kategori && m.requires.kategorier